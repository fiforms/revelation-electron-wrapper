...
const handoutWindow = {
  register(ipcMain, AppContext) {
    ...
    AppContext.callbacks['menu:help-contents-handout'] = () => {
      this.open(AppContext, 'readme', 'presentation.md');
    };
  },

  open(AppContext, slug, mdFile) {
    const tr = (text) => AppContext.translate(text);
    ...
    const requestedLanguage = AppContext.config.preferredPresentationLanguage || AppContext.config.language || 'en';
    ...
    const url = `http://${AppContext.hostURL}:${AppContext.config.viteServerPort}/presentations_${p_key}/${slug}/handout?${params.toString()}`;
            ...
            },
            {
                ...
                click: async () => {
                    ...
                    const { canceled, filePath } = await dialog.showSaveDialog({
                        title: tr('Save Handout as PDF'),
                        defaultPath: `${slug}-handout.pdf`,
                        filters: [{ name: tr('PDF Files'), extensions: ['pdf'] }]
                    });
