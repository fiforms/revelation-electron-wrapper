...
const { generateChallenge, verifyPeerChallenge, fingerprintPublicKey } = require('./peerAuth');
...
const mdnsManager = {
  ...
  refresh(AppContext) {
    const networkMode = AppContext.config.mode === 'network';
    const browseEnabled = AppContext.config.mdnsBrowse !== false;
    const publishRequested = AppContext.config.mdnsPublish === true;
    const discoveryEnabled = browseEnabled;
    const publishEnabled = networkMode && publishRequested;

    if (!discoveryEnabled && !publishEnabled) {
      this.stop(AppContext);
      return;
    }

    if (!this.ensureBonjour(AppContext)) return;

    if (discoveryEnabled) {
      this.ensureBrowser(AppContext);
    } else {
      if (this.browser) {
    ...
    if (publishEnabled) {
      this.ensureService(AppContext);
    } else if (this.service) {
      this.service.stop();
      this.service = null;
      AppContext.log('mDNS publish stopped (discovery remains active).');
    }
  },

  ensureBonjour(AppContext) {
    let Bonjour;
    try {
  ...
  ensureBrowser(AppContext) {
    ...
    AppContext.log(`mDNS browse started (${SERVICE_TYPE}).`);
  },

  ensureService(AppContext) {
    ...
    const needsRestart = !this.service ||
      this.service.name !== instanceName ||
    ...
    this.service = this.bonjour.publish({
      ...
      txt: {
        ...
        pairingPort,
        httpsEnabled: AppContext.config.httpsEnabled === true ? 'true' : 'false',
        pubKeyFingerprint: fingerprintPublicKey(AppContext.config.peerRsaPublicKey || '')
      }
    });
  ...
  emitPeers(AppContext) {
    ...
    windows.forEach((win) => {
      if (!win.isDestroyed()) {
        win.webContents.send('mdns-peers-updated', peers);
      }
    });
  }
};
...
async function verifyPairedMaster(AppContext, service, pairedMaster) {
  if (!pairedMaster?.peerPublicKey) {
    AppContext.error(
      `mDNS peer verification failed for ${pairedMaster?.instanceId || 'unknown'}: ` +
      'pairing predates the peer keypair split — unpair and pair again.'
    );
    return false;
  }
  ...
  const peerHttpsEnabled = service.txt?.httpsEnabled === 'true';
  const protocol = peerHttpsEnabled ? 'https' : 'http';

  const challenge = generateChallenge();
  try {
    const response = await fetchJSON(`${protocol}://${host}:${pairingPort}/peer/challenge`, {
      method: 'POST',
      body: { challenge, pin: pairedMaster.pairingPin }
    });
    if (!response.signature) {
      throw new Error('Missing signature.');
    }
    const ok = verifyPeerChallenge(pairedMaster.peerPublicKey, challenge, response.signature);
    if (!ok) {
