...
const mdnsManager = {
  ...
  handleServiceUp(AppContext, service) {
    ...
    if (instanceId && isPairedPeer(AppContext, instanceId)) {
...
function isPairedPeer(AppContext, instanceId) {
  return Array.isArray(AppContext.config.pairedMasters)
    && AppContext.config.pairedMasters.some((peer) => peer.instanceId === instanceId);
}
