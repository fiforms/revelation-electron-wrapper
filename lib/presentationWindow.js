...
function applyPresentationOptions(url, config) {
  try {
    const parsed = new URL(url);
    const lang = String(config.preferredPresentationLanguage || config.language || 'en').trim().toLowerCase();
    const variant = String(config.screenTypeVariant || '').trim().toLowerCase();
    if (lang) {
...
const presentationWindow = {
    ...
    suppressNextCloseBroadcast: false,
    ...
    async openWindow(AppContext, slug, mdFile, fullscreen) {
      ...
      const preload = isExternal && !shouldWrapInPip ? null : AppContext.preload;
      let options = {
      ...
      this.presWindow.on('closed', async () => {
        const shouldBroadcastClose = !this.suppressNextCloseBroadcast && !this.isRemote;
        this.suppressNextCloseBroadcast = false;

        if (shouldBroadcastClose) {
          // Send remote close event to peers
          const { sendPeerCommand } = require('./peerCommandClient');
          let shorturl = (presentationUrl || windowUrl).split('?')[0]; // Strip query params

          const command = { type: 'close-presentation', payload: { url: shorturl } };

          AppContext.log(`Presentation window closed, sending command to close on peers: ${presentationUrl || windowUrl}`);
          await sendPeerCommand(AppContext, command);
        }
    ...
    closeWindow(options = {}) {
        if (options.suppressPeerBroadcast) {
            this.suppressNextCloseBroadcast = true;
        }
        if (this.presWindow && !this.presWindow.isDestroyed()) {
