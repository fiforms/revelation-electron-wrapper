...
function buildInternalPresentationUrl(AppContext, slug, mdFile, overrides = {}) {
  ...
  const baseURL = buildServerURL(host, AppContext.config.viteServerPort, AppContext.config.httpsEnabled);
  let presentationUrl = `${baseURL}/presentations_${p_key}/${slug}/index.html`;
...
const presentationWindow = {
    ...
    resolveDisplay(displays, configuredIndex, configuredId, fallbackToPrimary = true) {
      if (configuredId) {
        const fingerprint = (d) => `${d.bounds.width}x${d.bounds.height}@${d.bounds.x},${d.bounds.y}`;
        const match = displays.find(d => fingerprint(d) === configuredId);
        if (match) return match;
      }
      if (Number.isFinite(configuredIndex) && displays[configuredIndex]) {
        return displays[configuredIndex];
      }
      return fallbackToPrimary ? screen.getPrimaryDisplay() : null;
    },

    async openWindow(AppContext, slug, mdFile, fullscreen, overrides = {}) {
      const isWindowed = String(AppContext?.config?.mainWindowMode || 'fullscreen').trim().toLowerCase() === 'windowed';
      const effectiveFullscreen = isWindowed ? false : !!fullscreen;
      const mainMuted = AppContext?.config?.mainWindowMuted === true;
      ...
      let targetDisplay = this.resolveDisplay(
        displays,
        AppContext.config.preferredDisplay,
        AppContext.config.preferredDisplayId
      );
      ...
      if (isExternal) {
        ...
        try {
          externalProtocol = new URL(slug).protocol;
        } catch {
          AppContext.error(`Refusing to open external presentation: malformed URL: ${slug}`);
          return;
        }
        if (externalProtocol !== 'http:' && externalProtocol !== 'https:') {
          AppContext.error(`Refusing to open external presentation with disallowed scheme: ${externalProtocol}`);
          return;
        }
      ...
      if (shouldWrapInPip) {
        ...
        const baseURL = buildServerURL(host, AppContext.config.viteServerPort, AppContext.config.httpsEnabled);
        windowUrl = `${baseURL}/pip.html?${params.toString()}`;
      }
      ...
      if (this.presWindow && !this.presWindow.isDestroyed()) {
        ...
        if (!keepMainWindowAlive && this.windowIsExternal !== null && this.windowIsExternal !== isExternal) {
          ...
          if (!isWayland) {
            if (isWindowed) {
              this.presWindow.setFullScreen(false);
            } else {
              ...
              this.presWindow.setFullScreen(effectiveFullscreen);
            }
          } else {
            this.presWindow.setFullScreen(effectiveFullscreen);
          }
          this.presWindow.webContents.setAudioMuted(mainMuted);
      ...
      if (isWayland) {
        ...
        options = {
          ...options,
          frame: isWindowed || !effectiveFullscreen,
          kiosk: !isWindowed && effectiveFullscreen
        };
      }
      else if (isWindowed) {
        options = {
    ...
    getAdditionalScreenSignature(config = {}) {
      return `${config.target || 'window'}:${config.displayIndex ?? 'none'}:${config.displayId || ''}:${config.language || ''}:${config.variant || ''}:${config.defaultMode || ''}:${config.defaultPresentation || ''}`;
    },
    ...
    getPublishFileInfo(AppContext) {
      ...
      const revUrl = `${baseURL}/${revRelativePath.replace(/\\/g, '/')}`;
      return {
        key: safeKey,
        filePath,
        revFilePath,
    ...
    async showDefaultOnMainPresentation(AppContext) {
      ...
      const mainMutedDefault = AppContext?.config?.mainWindowMuted === true;
      this.presWindow.webContents.setAudioMuted(mainMutedDefault);
      const isWindowedDefault = String(AppContext?.config?.mainWindowMode || 'fullscreen').trim().toLowerCase() === 'windowed';
      if (!isWindowedDefault && !this.presWindow.isFullScreen()) {
    ...
    createAdditionalScreenWindow(AppContext, screenConfig, idx) {
      ...
      const display = screenConfig.target === 'display'
        ? this.resolveDisplay(displays, screenConfig.displayIndex, screenConfig.displayId, false)
        : null;
      const useDisplay = !!display;
      if (screenConfig.target === 'display' && !display) {
        AppContext.log(`Additional screen ${idx + 1}: display unavailable, opening in window mode.`);
      }
      ...
      win.webContents.setAudioMuted(screenConfig.muted === true);
      win.setMenu(null);
