...
function register(routes, AppContext) {
  routes['POST /api/presentation/control'] = async (params, res, body) => {
    ...
    // Use deck.triggerKey() for all navigation actions to avoid double-processing from keydown/keyup
    // Special cases for overview and blank which need direct method calls
    if (action === 'overview') {
      ...
      // Use triggerKey instead of sendKeyToPresentation to avoid double navigation
      await pres.webContents.executeJavaScript(`window.deck && window.deck.triggerKey?.(${binding.keyCode})`);
    }
