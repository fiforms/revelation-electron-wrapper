...
const { DEFAULT_VITE_PORT, DEFAULT_API_PORT } = require('./serverManager');
...
const defaultPlugins = ["addmedia","bibletext","hymnary","virtualbiblesnapshots", "resources", "mediafx", "compactor", "richbuilder", "slidesorter", "mdvalidate", "divideslides"];

// Both of these are security secrets, so they must come from a CSPRNG — never
// Math.random(), whose internal state is recoverable from observed output.
// See revelation/doc/SECURITY.md (F1).

// The server access key gates /presentations_<key>/, /plugins_<key>/,
// /thumbs_<key>/ and the control API on :8001. 16 lowercase hex chars (64 bits)
// keeps it in the same character class the old base36 keys used, so it stays
// safe to interpolate into folder names, URL paths and regexes.
function generateAccessKey() {
  return crypto.randomBytes(8).toString('hex');
}

// 6-digit mDNS pairing PIN. Brute force is bounded by the 3-attempts-per-60s
// lockout in vite.plugins.js, but the PIN still must not be predictable.
...
const defaultConfig = {
  ...
  viteServerPort: DEFAULT_VITE_PORT,
  apiServerPort: DEFAULT_API_PORT,
  apiServerEnabled: true,
  httpsEnabled: false,
  // Addresses of the public relay. These are *not* used by presentations
  // running inside the app unless useRemotePublicServer is turned on — they
  // are the values baked into standalone exports (which have no local server)
  // and the preset offered for off-LAN remote control.
  ...
  // Route this app's own realtime traffic (Reveal Remote + presenter plugins)
  // through the public relay instead of this machine's Vite server. Off by
  // default: local presentations should stay on the LAN. Turn on only when a
  // remote control or viewer cannot reach this network — a phone on cellular,
  // say. See revelation/doc/SECURITY.md (F3).
  useRemotePublicServer: false,
  ...
  mdnsPairingPin: null,
  // WordPress publishing identity. Used only by plugins/wordpress_publish.
  rsaPublicKey: null,
  rsaPrivateKey: null,
  // Peer pairing identity. Deliberately separate from the WordPress keypair so
  // that /peer/challenge — which signs caller-supplied bytes — cannot be used
  // to mint signatures a WordPress site will honour. See revelation/doc/SECURITY.md (F2).
  peerRsaPublicKey: null,
  peerRsaPrivateKey: null,
  pairedMasters: [],
...
function loadConfig() {
  ...
  if (fs.existsSync(targetConfigPath)) {
    try {
      ...
      // Repair configs poisoned by the old fallback behaviour: when Vite could
      // not get its port it took the next free one — often the API server's —
      // and that value was persisted, so every later launch collided too. The
      // symptom is the API server answering the main window with "Not Found".
      // Nothing can start correctly while these two are equal.
      if (
        Number(loadedConfig.apiServerPort) === Number(loadedConfig.viteServerPort) &&
        Number.isFinite(Number(loadedConfig.viteServerPort))
      ) {
        const clashed = loadedConfig.apiServerPort;
        loadedConfig.apiServerPort = DEFAULT_API_PORT;
        saveConfig(loadedConfig);
        console.warn(
          `Vite and API server ports were both ${clashed}; moved API server to ${DEFAULT_API_PORT}`
        );
      }
      if (!loadedConfig.key || typeof loadedConfig.key !== 'string') {
        loadedConfig.key = generateAccessKey();
        saveConfig(loadedConfig);
      ...
      if (loadedConfig.mdnsPublish && !loadedConfig.mdnsPairingPin) {
        loadedConfig.mdnsPairingPin = generatePairingPin();
        saveConfig(loadedConfig);
      ...
      if (!loadedConfig.rsaPublicKey || !loadedConfig.rsaPrivateKey) {
        ...
        saveConfig(loadedConfig);
        console.warn('Generated RSA keypair for WordPress publishing');
      }
      if (!loadedConfig.peerRsaPublicKey || !loadedConfig.peerRsaPrivateKey) {
        const { publicKey, privateKey } = generateKeyPair();
        loadedConfig.peerRsaPublicKey = publicKey;
        loadedConfig.peerRsaPrivateKey = privateKey;
        saveConfig(loadedConfig);
        console.warn('Generated RSA keypair for peer pairing');
      }
  ...
  if (!config.peerRsaPublicKey || !config.peerRsaPrivateKey) {
    const { publicKey, privateKey } = generateKeyPair();
    config.peerRsaPublicKey = publicKey;
    config.peerRsaPrivateKey = privateKey;
  }
...
function _buildSaveableObject(config) {
  ...
  // A server that could not get its configured port falls back to another one
  // for the session and writes the live value into config so URL builders see
  // it. Persist what the user chose, not the fallback — otherwise one transient
  // conflict silently rewrites their settings, and the rewritten value can
  // collide with the other server on the next launch.
  if (Number.isFinite(Number(out.configuredViteServerPort))) {
    out.viteServerPort = Number(out.configuredViteServerPort);
  }
  if (Number.isFinite(Number(out.configuredApiServerPort))) {
    out.apiServerPort = Number(out.configuredApiServerPort);
  }
  delete out.configuredViteServerPort;
  delete out.configuredApiServerPort;
  ...
  if (Array.isArray(out.pairedMasters)) {
    out.pairedMasters = out.pairedMasters
      ...
      .map((item) => ({
        ...
        name: item.name,
        // The master's peer identity. The legacy `publicKey` field is
        // deliberately not carried forward: it held the master's WordPress key,
        // which is no longer what peer signatures are made with (revelation/doc/SECURITY.md
        // F2). Entries without `peerPublicKey` are detected as stale and the
        // user is asked to pair again.
        peerPublicKey: item.peerPublicKey,
        peerProtocol: item.peerProtocol,
        pairedAt: item.pairedAt,
        hostHint: item.hostHint,
        pairingPortHint: item.pairingPortHint,
        httpsEnabled: item.httpsEnabled === true,
...
function normalizeAdditionalScreens(rawList) {
  ...
  return rawList
    .map((item) => {
      ...
      const displayId = typeof item.displayId === 'string' ? item.displayId.trim() : '';
      ...
      if (target === 'display' && displayIndex === null && !displayId) return null;
...
module.exports = { loadConfig, saveConfig, saveConfigAsProfile, setActiveProfile, listProfiles, validateProfileName, generateAccessKey, generatePairingPin, configPath, profilesDir };
