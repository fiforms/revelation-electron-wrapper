...
const defaultConfig = {
  ...
  ffmpegPath: null,
  ffprobePath: null,
  plugins: ["addmedia","bibletext","hymnary","virtualbiblesnapshots", "resources", "mediafx", "compactor", "credit_ccli", "richbuilder", "slidesorter", "wordpress_publish"],
  pluginConfigs: {},
  language: 'en',
...
function loadConfig() {
  ...
  const loadedConfig = JSON.parse(fs.readFileSync(configPath, 'utf-8'));
      ...
      if (!loadedConfig.plugins.includes('credit_ccli')) {
        loadedConfig.plugins.push('credit_ccli');
        saveConfig(loadedConfig);
      }
      if (!loadedConfig.plugins.includes('wordpress_publish')) {
        loadedConfig.plugins.push('wordpress_publish');
        saveConfig(loadedConfig);
      }
...
function saveConfig(config) {
  ...
  delete saveConfig.virtualPeersAlwaysOpen;
