...
const { SYNC_CONFLICTS_DIRNAME } = require('./presentationSyncPeers');
...
const exportPresentation = {
  ...
  register(ipcMain, AppContext) {
    ...
    ipcMain.handle('export-presentation', async (event, slug, includeMedia, options = {}) => {
      ...
      const createStandalone = options?.createStandalone === true;
      const showSplashscreen = createStandalone && options?.showSplashscreen !== false;
      const useRevealRemotePublicServer = createStandalone && options?.useRevealRemotePublicServer !== false;
      ...
      return await this.run(folderPath, slug, includeMedia, showSplashscreen, createStandalone, event);
    });
  },

  // Open the export presentation window
  async run(folderPath, slug, includeMedia, showSplashscreen, createStandalone, event) {
    ...
    try {
      ...
      const manifest = await buildPresentationManifest(folderPath, {
        appVersion,
      ...
      await new Promise((resolve, reject) => {
        ...
        archive.pipe(output);
        // Exclude .thumbs — builder-only thumbnail cache, easily regenerated.
        // Exclude .sync-conflicts — local backups from WordPress sync conflict resolution.
        archive.glob('**/*', {
          cwd: folderPath,
          dot: true,
          skip: ['**/.thumbs', SYNC_CONFLICTS_DIRNAME]
        });
        archive.finalize();
      });
      ...
      if (fs.existsSync(folderPath)) {
        try {
          ...
          await writePresentationManifest(folderPath, {
