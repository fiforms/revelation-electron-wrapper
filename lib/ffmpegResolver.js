...
/**
 * Resolves the ffprobe binary path with a three-level fallback chain:
 * 1. User-configured path (AppContext.config.ffprobePath)
 * 2. Packaged binary location (process.resourcesPath/ffmpeg/) — same folder as ffmpeg
 * 3. System PATH
 */
async function resolveFfprobeBinary(AppContext) {
  if (AppContext?.config?.ffprobePath) {
    return AppContext.config.ffprobePath;
  }
