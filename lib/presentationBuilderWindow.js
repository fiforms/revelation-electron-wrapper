    ...
    if (variantLanguage) {
      try {
        ...
        const availableLanguages = Array.isArray(session?.availableSpellCheckerLanguages)
          ? session.availableSpellCheckerLanguages
          : [];
        const resolvedLanguage = resolveSpellcheckerLanguage(session, variantLanguage);
        if (resolvedLanguage) {
          AppContext.log(
            `[builder spellcheck] Requested language "${variantLanguage}" is available as "${resolvedLanguage}".`
          );
          session.setSpellCheckerLanguages([resolvedLanguage]);
        } else {
          const availableText = availableLanguages.length
            ? availableLanguages.join(', ')
            : '(none)';
          AppContext.log(
            `[builder spellcheck] Requested language "${variantLanguage}" is not available. Available dictionaries: ${availableText}`
          );
        }
      } catch (err) {
        AppContext.error('Failed to set spellchecker language:', err.message);
      }
    }
