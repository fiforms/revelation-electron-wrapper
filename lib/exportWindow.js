...
async function captureSlidesToImageFolder(AppContext, slug, mdFile, width, height, delay, exportDir, maxSlides = null) {
  ...
  try {
    ...
    const plannedSlides = Number.isFinite(Number(maxSlides)) && Number(maxSlides) > 0
      ? Math.min(totalSlides, Math.floor(Number(maxSlides)))
      : totalSlides;
    AppContext.log(`📸 Exporting ${plannedSlides} slide(s) as images...`);
