...
const pluginDirector = {
    ...
    register(ipcMain, AppContext) {
        ...
        AppContext.callbacks['menu:install-plugin-zip'] = async () => {
            ...
            try {
                // Determine plugin name (zip root folder or filename)
                const pluginName = require('path').basename(zipPath, '.zip');
                const destPath = require('path').join(pluginFolder, pluginName);
                ...
                // Extract ZIP into plugins folder
                await new Promise((resolve, reject) => {
                fs.createReadStream(zipPath)
                    .pipe(require('unzipper').Extract({ path: destPath }))
                    .on('close', resolve)
                    .on('error', reject);
                });
