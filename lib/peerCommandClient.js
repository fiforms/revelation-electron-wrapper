...
const { verifyPeerSocketPayload } = require('./peerAuth');
...
function shouldRewriteForMaster(master, selection, parsedUrl) {
  if (!isPrivateOrLocalHost(parsedUrl.hostname)) return { enabled: false, reason: 'external-url' };
...
function handlePeerCommand(AppContext, command, sourceMasterId = 'unknown') {
  ...
  const mainWindowPeerControlEnabled = AppContext?.config?.mainWindowOpenOnPeerPush !== false;
  ...
  switch (command.type) {
    case 'open-presentation': {
      ...
      let peerUrlProtocol;
      try {
        peerUrlProtocol = new URL(url).protocol;
      } catch {
        AppContext.error(`Rejecting peer open-presentation command with malformed URL from ${sourceMasterId}.`);
        return;
      }
      if (peerUrlProtocol !== 'http:' && peerUrlProtocol !== 'https:') {
        AppContext.error(`Rejecting peer open-presentation command with disallowed scheme '${peerUrlProtocol}' from ${sourceMasterId}.`);
        return;
      }
      const resolvedUrl = maybeRewritePeerPresentationUrl(AppContext, sourceMasterId, url);
      if (mainWindowPeerControlEnabled) {
    ...
    case 'navigate-slide': {
      if (!mainWindowPeerControlEnabled) break;
...
const peerCommandClient = {
  ...
  async refreshConnection(AppContext) {
    ...
    const connectTasks = masters.map(async (master) => {
      ...
      const selection = resolveMasterConnection(AppContext, master);
      if (!masterId || !selection?.host || !selection?.port) {
        this.setMasterWarning(
          AppContext,
          masterId,
          'offline-missing-peer-info',
          `Peer command master ${masterId || 'unknown'} appears to be offline (missing host or port).`
        );
        if (masterId) this.disconnectMaster(masterId);
        return;
      }

      // Pairings made before the peer keypair split store only the legacy
      // `publicKey`, which the master no longer signs with (revelation/doc/SECURITY.md F2).
      // Say so plainly instead of surfacing a signature failure every refresh.
      if (!master.peerPublicKey) {
        this.setMasterWarning(
          AppContext,
          masterId,
          'stale-pairing-protocol',
          `Pairing with master ${master.name || masterId} was made with an older version and must be renewed — unpair and pair again.`
        );
        this.disconnectMaster(masterId);
        return;
      }
      ...
      const endpointKey = `${selection.host}:${selection.port}:${master.peerPublicKey}:${master.pairingPin || ''}`;
      const existing = this.connections.get(masterId);
      ...
      const masterHttpsEnabled = master.httpsEnabled === true;
      const masterProtocol = masterHttpsEnabled ? 'https' : 'http';
      try {
        const socketInfo = await fetchJSON(
          `${masterProtocol}://${selection.host}:${selection.port}${SOCKET_INFO_ENDPOINT}?instanceId=${encodeURIComponent(AppContext.config.mdnsInstanceId)}${pinParam}`
        );
        ...
        const ok = verifyPeerSocketPayload(master.peerPublicKey, payload, signature);
        if (!ok) {
