...
function handlePeerCommand(AppContext, command, sourceMasterId = 'unknown') {
  ...
  switch (command.type) {
    case 'open-presentation': {
      ...
      presentationWindow.openAdditionalScreensForPeerUrl(AppContext, url);
      break;
    }
  ...
  async refreshConnection(AppContext) {
    ...
    const connectTasks = masters.map(async (master) => {
      ...
      socket.on('connect_error', (err) => {
        ...
        this.showToast(
          AppContext,
          `peer-connect-error:${masterId}:${err.message}`,
    ...
    results.forEach((result) => {
      if (result.status === 'rejected') {
        ...
        this.showToast(AppContext, `peer-refresh-error:${msg}`, `Master connection error: ${msg}`, 30000);
      }
    });
  }
};
