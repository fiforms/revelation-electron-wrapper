...
function pickMaster(AppContext) {
  ...
  const withHint = masters.find((master) => master.hostHint && master.pairingPortHint);
  if (withHint) {
    return {
      master: withHint,
      host: withHint.hostHint,
      port: withHint.pairingPortHint
    };
  }
...
function handlePeerCommand(AppContext, command) {
  ...
  AppContext.log(`Received peer command: ${command.type} from master`);
  ...
  switch (command.type) {
    case 'open-presentation': {
      ...
      if(presentationWindow.presWindow && !presentationWindow.presWindow.isDestroyed()) {
        if(presentationWindow.isRemote) {
          AppContext.log('Closing existing remote presentation window before opening new one.');
          presentationWindow.closeWindow();
        }
        else {
          AppContext.log('Presentation window already open locally — ignoring remote open command.');
          return;
        }
      }
    ...
    case 'close-presentation': {
      ...
      else if(!presentationWindow.isRemote) {
        AppContext.log('Presentation window is local — ignoring remote close command.');
      }
      else if(presentationWindow.presWindow.webContents.getURL().split('?')[0] !== url) {
        AppContext.log('Presentation window URL does not match — ignoring remote close command.');
      }
      else {
        presentationWindow.closeWindow();
      }
...
const peerCommandClient = {
  ...
  async refreshConnection(AppContext) {
    ...
    const socketInfo = await fetchJSON(
      `http://${host}:${port}${SOCKET_INFO_ENDPOINT}?instanceId=${encodeURIComponent(AppContext.config.mdnsInstanceId)}${pinParam}`
    );
