...
function maybeRewritePeerPresentationUrl(AppContext, sourceMasterId, url) {
  ...
  if (!master?.natCompatibility) return url;
  ...
  try {
    const parsed = new URL(url);
    parsed.hostname = selection.host;
    parsed.port = String(selection.port);
    const rewritten = parsed.toString();
    if (rewritten !== url) {
      AppContext.log(`NAT compatibility rewrote peer URL for ${sourceMasterId}: ${url} -> ${rewritten}`);
    }
    return rewritten;
  } catch (err) {
    AppContext.error(`NAT compatibility URL rewrite failed for ${sourceMasterId}: ${err.message}`);
    return url;
  }
}
