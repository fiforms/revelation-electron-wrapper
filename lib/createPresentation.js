...
const createPresentation = {
  ...
  open(AppContext, slug = null, mdFile = null) {
    ...
    const url = slug && mdFile ? `/admin/edit-metadata.html?dir=presentations${AppContext.config.key}&slug=${slug}&md=${mdFile}` : '/admin/create.html';
  ...
  run(data, AppContext, userSlug = null, userMDFile = null) {
    ...
    if(!userSlug && !userMDFile) {
      ...
      fs.copyFileSync(path.join(AppContext.config.revelationDir,templateDir, 'thumbnail.webp'), path.join(presDir, 'thumbnail.webp'));
