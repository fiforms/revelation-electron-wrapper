...
const createPresentation = {
  ...
  open(AppContext, slug = null, mdFile = null) {
    ...
    const url = slug && mdFile ? `/admin/edit-metadata.html?dir=${AppContext.config.presentationsDir}&slug=${slug}&md=${mdFile}` : '/admin/create.html';
  ...
  run(data, AppContext, userSlug = null, userMDFile = null) {
    const presentationsDir = path.join(AppContext.config.revelationDir,AppContext.config.presentationsDir);
    let mdBody = '';
    let slug = userSlug;
