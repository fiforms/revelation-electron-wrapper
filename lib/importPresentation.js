...
const importPresentation = {
  ...
  async runZipImport(payload, AppContext) {
    ...
    if (fs.existsSync(bundledManifestPath)) {
      ...
      if (manifestData) {
        const { errors, checked } = await validateAgainstManifest(destPath, manifestData);
        if (errors.length > 0) {
  ...
  async runUrlImport(payload, AppContext) {
    ...
    try {
      for (const manifestEntry of manifest.files) {
        ...
        if (relPath !== 'manifest.json' && manifestEntry && typeof manifestEntry === 'object') {
          ...
          if (expectedSize !== undefined || expectedSha1) {
            ...
            if (expectedSize !== undefined && data.length !== expectedSize) {
              validationErrors.push({
                filename: relPath,
                issue: `Size mismatch: expected ${expectedSize} B, got ${data.length} B`
              });
...
async function validateAgainstManifest(destPath, manifestData) {
  const errors = [];
  let checked = 0;
...
function fetchText(url) {
  return fetchBinary(url).then((buffer) => buffer.toString('utf8'));
}
