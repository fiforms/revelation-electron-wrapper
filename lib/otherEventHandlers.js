...
function ensureReadmeTemplate(presentationsDir) {
  const templateReadmePath = path.resolve(__dirname, '../revelation/templates/readme');
  ...
  return {
    readmePresDir,
    readmePresentationPath: path.join(readmePresDir, 'presentation.md'),
...
const otherEventHandlers = {
    register(ipcMain, AppContext) {
        ...
        AppContext.callbacks['menu:regenerate-readme'] = async () => {
          const {
            readmePresentationPath,
            readmeYamlPath
          } = ensureReadmeTemplate(AppContext.config.presentationsDir);
          const projectReadmePath = path.resolve(__dirname, '../revelation/README.md');
          const referencePath = path.resolve(__dirname, '../revelation/doc/REFERENCE.md');
          ...
          try {
            ...
            if (!fs.existsSync(projectReadmePath)) {
              throw new Error(`Missing source README at ${projectReadmePath}`);
            }
            ...
            AppContext.log(`📝 Regenerated ${readmePresentationPath}`);

            await dialog.showMessageBox({
              type: 'info',
              title: 'README',
              message: 'README presentation regenerated.',
              detail: readmePresentationPath
            });
          } catch (err) {
            AppContext.error(`README regeneration failed: ${err.message}`);
            await dialog.showMessageBox({
              type: 'error',
              title: 'README',
              message: 'Failed to regenerate README presentation.',
              detail: err.message
            });
          }
        }
