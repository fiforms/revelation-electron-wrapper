...
const fs = require('fs');
const { saveConfig, generateAccessKey, generatePairingPin } = require('./configManager');
    ...
    register(ipcMain, AppContext) {
        ...
        ipcMain.on('open-external-url', (_event, href) => {
            ...
            try {
                ...
                AppContext.error('[main] Rejected malformed external URL:', href);
                return;
            }
            const allowedProtocols = new Set(['http:', 'https:', 'mailto:']);
            if (!allowedProtocols.has(parsed.protocol)) {
                AppContext.error('[main] Rejected external URL with disallowed scheme:', parsed.protocol);
                return;
            }
            AppContext.log('[main] Opening external URL:', parsed.toString());
        ...
        ipcMain.handle('get-app-config', () => {
          ...
          delete safeConfig.peerRsaPrivateKey;
        ...
        ipcMain.handle('list-presentation-images', async (_event, slug) => {
          ...
          for (const entry of entries) {
            ...
            if (filename.toLowerCase().endsWith('.thumb.jpg')) continue;
        ...
        ipcMain.handle('save-app-config', (_event, updates) => {
          ...
          if (enablingMdns && !providedPin && !existingPin) {
            updates.mdnsPairingPin = generatePairingPin();
          }
          // An explicit port from Settings replaces any remembered preference
          // left behind by a session that had to fall back to another port —
          // otherwise saveConfig would write the stale value back over it.
          if (Object.prototype.hasOwnProperty.call(updates || {}, 'viteServerPort')) {
            delete AppContext.config.configuredViteServerPort;
          }
          if (Object.prototype.hasOwnProperty.call(updates || {}, 'apiServerPort')) {
            delete AppContext.config.configuredApiServerPort;
          }
        ...
        ipcMain.handle('get-display-list', () => {
          ...
          return screen.getAllDisplays().map(d => ({
            ...d,
            fingerprint: `${d.bounds.width}x${d.bounds.height}@${d.bounds.x},${d.bounds.y}`
          }));
        });
        ...
        AppContext.callbacks['menu:show-library'] = () => {
          ...
          const url = `${baseURL}/media-library.html?key=${key}`
        ...
        AppContext.callbacks['menu:show-presentation-list'] = () => {
          const key = AppContext.config.key;
          const baseURL = buildServerURL(AppContext.hostURL, AppContext.config.viteServerPort, AppContext.config.httpsEnabled);
          const url = `${baseURL}/presentations.html?key=${key}`
          AppContext.win.loadURL(url);
        }
        ...
        // File selection and editing for external macro files
        ipcMain.handle('select-macro-file', async (event, slug) => {
          ...
          try {
            const presentationDir = path.join(AppContext.config.presentationsDir, slug);
            const result = await dialog.showOpenDialog(win, {
              defaultPath: presentationDir,
              filters: [
            ...
            // Check if the file is within the presentation directory
            const isWithinPresDir = selectedPath.startsWith(presentationDir + path.sep) || selectedPath === presentationDir;

            // Return relative path if within presentation dir, absolute path otherwise
            const filename = isWithinPresDir
              ? path.relative(presentationDir, selectedPath).replace(/\\/g, '/')
              : selectedPath;

            return { success: true, filename, isAbsolute: !isWithinPresDir };
          } catch (err) {
            return { success: false, error: err.message };
          }
        });

        ipcMain.handle('open-file-with-editor', async (event, relativePath, slug) => {
          try {
            let fullPath = relativePath;

            // If slug is provided and path is relative, construct absolute path
            if (slug && !relativePath.startsWith('/')) {
              fullPath = path.join(AppContext.config.presentationsDir, slug, relativePath);
            }

            AppContext.log(`[open-file-with-editor] Opening: ${fullPath}`);
        ...
        ipcMain.handle('save-macros-to-file', async (event, macrosObject, slug, suggestedFilename = 'macros.yaml') => {
          const win = BrowserWindow.fromWebContents(event.sender);
          if (!win || win.isDestroyed()) {
            return { success: false, error: 'No window context' };
          }
          ...
          try {
            ...
            const result = await dialog.showSaveDialog(win, {
              defaultPath: path.join(presentationDir, suggestedFilename),
              filters: [
                { name: 'YAML Files', extensions: ['yaml', 'yml'] },
                { name: 'All Files', extensions: ['*'] }
              ]
            ...
            // Convert macros object to YAML format
            const yaml = require('js-yaml');
            const yamlContent = yaml.dump(macrosObject, { lineWidth: -1 });

            // Write the file
            fs.writeFileSync(filePath, yamlContent, 'utf8');

            // Return relative path from presentation directory
            const relativePath = path.relative(presentationDir, filePath).replace(/\\/g, '/');
            return { success: true, filename: relativePath, fullPath: filePath };
          } catch (err) {
            return { success: false, error: err.message };
          }
        });

        ipcMain.handle('load-macros-from-file', async (event, filePath) => {
          try {
            const yaml = require('js-yaml');
            ...
            // Extract macros (supports both 'macros' key and flat structure for backwards compatibility)
