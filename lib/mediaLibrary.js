    ...
    async hashAndStore(passedFilePath, metadata, AppContext, relatedFile = null) {
        ...
        if(passedFilePath) {
          ...
          const { canceled, filePaths } = await dialog.showOpenDialog({
            ...
            filters: [{ name: 'Media Files', extensions: ['jpg','jpeg','png','gif','avif','mp4','mov','avi','webp','mp3','ogg','wav','m4a','aac','opus','svg'] }],
        ...
        for(const filePath of filePathArray) {
          ...
          metadata['thumbnail'] = mediaType(newFilename) === 'audio' ? '' : path.basename(thumbPath);
    ...
    https.get(url, (res) => {
      ...
      if (!filename) {
          try {
            ...
            filename = path.basename(parsed.pathname) || 'downloaded';
          } catch {
            filename = 'downloaded';
          }
      }
      ...
      if (!path.extname(filename)) {
        ...
        else if (contentType.includes('audio/wav') || contentType.includes('audio/x-wav')) filename += '.wav';
        else if (contentType.includes('audio/mp4') || contentType.includes('audio/aac')) filename += '.m4a';
