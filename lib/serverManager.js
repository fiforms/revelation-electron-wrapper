...
const serverManager = {
    ...
    async startServers(mode, AppContext) {
        ...
        const remotePortResult = mode === 'network'
            ? await findAvailablePort(AppContext.config.revealRemoteServerPort, 20)
            : { port: AppContext.config.revealRemoteServerPort, changed: false };
        AppContext.currentMode = mode;
        ...
        if (remotePortResult.changed) {
            AppContext.log(`⚠️ Reveal Remote port ${AppContext.config.revealRemoteServerPort} was in use; using ${remotePortResult.port} for this session.`);
            AppContext.config.revealRemoteServerPort = remotePortResult.port;
        }
        ...
        if (mode === 'network') {
            writeRevealRemoteJSFile(
              AppContext.config.revealRemoteServerPort,
