...
const serverManager = {
    ...
    async startServers(mode, AppContext) {
        ...
        let ffmpegBin = await resolveFfmpegBin(AppContext);
        if (ffmpegBin) {
            env.FFMPEG_BIN = ffmpegBin;
            AppContext.log(`✅ Using ffmpeg binary at: ${ffmpegBin}`);
        } else {
            AppContext.log('⚠️ ffmpeg binary not found. Video/audio features may not work. Configure ffmpegPath in settings or ensure ffmpeg is packaged correctly.');
        }
...
async function resolveFfmpegBin(AppContext) {
  // Use configured path if available
  if (AppContext?.config?.ffmpegPath) {
    return AppContext.config.ffmpegPath;
  }

  // Fall back to packaged binary location (set up by electron-builder from ffmpeg-static)
  ...
  // Fall back to system path (hoping it's installed globally)
  try {
    // Searches the system PATH for 'ffmpeg'
    const resolvedPath = await which('ffmpeg');
    return resolvedPath;
  } catch (err) {
    // ffmpeg not found in PATH
  }
  
  console.log(`ffmpeg binary not found. Configure ffmpegPath in settings or ensure ffmpeg is packaged at ${packagedPath}`);
  return null; // Let the Vite server handle the error if ffmpeg is required but not found
}

module.exports = { serverManager, findAvailablePort };
