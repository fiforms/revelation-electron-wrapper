...
function generatePluginsIndexMarkdown(plugins) {
  if (plugins.length === 0) {
    return '# Plugin Documentation Index\n\nNo plugins are currently installed.\n';
  }

  const lines = [
    '# Plugin Documentation Index',
    '',
    `This documentation covers all ${plugins.length} installed plugins for REVELation Snapshot Presenter.`,
    ''
  ];

  for (const plugin of plugins) {
    lines.push(`## [${plugin.title}](plugins/${plugin.dirName}/README.md)`);
...
function getDefaultDocSources({ wrapperRoot, revelationDir }) {
  ...
  const pluginIndexPath = path.join(wrapperRoot, '.temp-plugin-index.md');
  const pluginIndexContent = generatePluginsIndexMarkdown(pluginManifests);
  fs.mkdirSync(path.dirname(pluginIndexPath), { recursive: true });
  fs.writeFileSync(pluginIndexPath, pluginIndexContent, 'utf-8');
  ...
  return [
    ...
    { key: 'doc/PLUGIN_INDEX.md', absPath: pluginIndexPath },
    ...pluginReadmes,
...
function buildRelativeMarkdownHref(fromOutputPath, toOutputPath, hash = '') {
  const fromDir = path.posix.dirname(String(fromOutputPath || 'presentation.md'));
  let relativePath = path.posix.relative(fromDir, String(toOutputPath || 'presentation.md'));
  if (!relativePath || relativePath === '') {
    relativePath = path.posix.basename(String(toOutputPath || 'presentation.md'));
  }
...
function generateDocumentationPresentations({ presentationsDir, revelationDir, wrapperRoot, appVersion = '' }) {
  ...
  // Clean up temporary plugin index file
  const tempPluginIndexPath = path.join(wrapperRoot, '.temp-plugin-index.md');
  if (fs.existsSync(tempPluginIndexPath)) {
    try {
      fs.unlinkSync(tempPluginIndexPath);
    } catch {
