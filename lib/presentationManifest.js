...
function shouldIncludeInManifest(relPathPosix) {
  ...
  if (relPathPosix.split('/').includes(THUMBS_DIRNAME)) return false;
...
async function collectManifestFiles(presentationDir, { forceRecompute = false } = {}) {
  ...
  const walk = (dirPath) => {
    for (const entry of fs.readdirSync(dirPath, { withFileTypes: true })) {
      ...
      if (entry.isDirectory()) {
        if (entry.name === THUMBS_DIRNAME) continue;
        walk(absolutePath);
        continue;
      }
...
async function buildPresentationManifest(presentationDir, data = {}, { forceRecompute = false } = {}) {
  return {
    ...data,
    files: await collectManifestFiles(presentationDir, { forceRecompute })
  };
}
