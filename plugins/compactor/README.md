...
4. Optionally converts PNG files to `webp`, `avif` (`AVIF (Better)`), or `avif` via fast mode (`AVIF (Faster)`).
5. Optionally converts JPG/JPEG files to `webp`, `avif` (`AVIF (Better)`), or `avif` via fast mode (`AVIF (Faster)`).
6. Optionally compacts video assets with the same max dimensions and a configurable quality level.
7. Optionally removes files in the copied presentation folder when they are not referenced by markdown.
8. Rewrites relative media references in copied `.md` files when converted image filenames/extensions change.
...
- Remove unreferenced files: disabled by default
