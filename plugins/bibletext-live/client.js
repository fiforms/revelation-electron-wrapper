(function () {
  ...
  window.RevelationPlugins['bibletext-live'] = {
    name: 'bibletext-live',
    priority: 89,
      ...
      if (ctx?.baseURL) {
        ...
        if (typeof window.loadTranslations === 'function') {
          window.loadTranslations().catch((err) => {
            console.warn('[bibletext-live] failed to load plugin translations:', err);
          });
        }
      }
    ...
    _startLiveFollower() {
      ...
      if (!endpoint || !roomId) {
        console.warn('[bibletext-live] live verse follower disabled: no socket endpoint or room');
        return;
      }
      ...
      socket.on('connect', () => {
        socket.emit('presenter-plugin:join', { plugin: 'bibletext-live', roomId }, (res = {}) => {
          if (res.ok) {
            socket.emit('presenter-plugin:event', { type: 'live-verse-request', payload: {} });
          } else {
            console.warn('[bibletext-live] live room join failed:', res.error || 'unknown error');
          }
        });
      });
    ...
    //
    // buildLiveVerseHtml() in ../bibletext-live/plugin.js emits an exactly known
    // vocabulary — div/p/span/em/br carrying only `bibletext-live*` class names —
    // so this is an allowlist rather than the general markdown sanitizer. That is
    // stricter than sanitizeRenderedHTML(): it also rejects markup the general
    // sanitizer deliberately permits, such as <img>, <iframe>, and the inline
    // `style` that would let an injected element cover the projected screen.
    //
